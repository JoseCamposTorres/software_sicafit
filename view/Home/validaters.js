$(document).ready(function () {
  ...
  function buscarUsuario(dniInput) {
    ...
    let apellidoInput = dniInput.closest(".detenido-row").find(".apellido-input");
    let timerInterval;
...
$(document).on("input", ".edad-input", function () {
    ...
    let apellidoCompleto = apellidoInput.data("full-lastname") || apellidoInput.val();

    // Guardar nombres completos si no están almacenados
    if (!nombreInput.data("full-name")) nombreInput.data("full-name", nombreCompleto);
    if (!apellidoInput.data("full-lastname")) apellidoInput.data("full-lastname", apellidoCompleto);
    ...
    if (edad === "" || parseInt(edad, 10) >= 18) {
        ...
        let inicialNombre = nombreCompleto.split(" ").map(n => n.charAt(0)).join(". ") + ".";
        let inicialApellido = apellidoCompleto.split(" ").map(a => a.charAt(0)).join(". ") + ".";
...
document.addEventListener("DOMContentLoaded", function () {
  let fechaInput = document.getElementById("caso_date");
  let horaInput = document.getElementById("caso_hour");
  // Restringe las fechas futuras
